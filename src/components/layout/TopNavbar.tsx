...
import { Bell, Search, LogOut, Moon, Sun, ChevronRight, Menu, X, User, LayoutDashboard, Users, Puzzle, RotateCcw, FileText, Settings } from 'lucide-react';
          ...
          <nav className="hidden md:flex items-center gap-1 text-sm text-muted-foreground" aria-label="Breadcrumb">
            ...
            {breadcrumbs.map((crumb, i) => (
              <span key={i} className="flex items-center gap-1">
                ...
                <span className={i === breadcrumbs.length - 1 ? 'text-foreground font-medium capitalize' : 'capitalize'}>{crumb}</span>
              </span>
            ))}
          </nav>
              ...
              {showSearch && (
                <div className="absolute right-0 top-12 w-80 bg-card border border-border rounded-xl card-shadow-lg p-2 animate-fade-in">
                  <input
                    autoFocus
                    type="text"
                    placeholder="Search users, features, pages..."
                    value={searchQuery}
                    onChange={e => setSearchQuery(e.target.value)}
                    className="w-full px-3 py-2 bg-muted rounded-lg text-sm text-foreground placeholder:text-muted-foreground outline-none"
                    ...
                    <div className="mt-2 space-y-1">
                      {searchResults.map((item, i) => (
                        <button
                          key={i}
                          onClick={() => {
                            if (item.type === 'page') navigate(item.sublabel);
                            setShowSearch(false);
                            setSearchQuery('');
                          }}
                          className="flex items-center justify-between w-full px-3 py-2 rounded-lg hover:bg-muted text-left text-sm transition-colors"
                        >
                  ...
                  <div className="max-h-64 overflow-y-auto">
                    {notifs.map(n => (
                      <button
                        ...
                        className={cn('w-full text-left px-4 py-3 hover:bg-muted transition-colors border-b border-border last:border-0', !n.read && 'bg-primary/5')}
                      >
